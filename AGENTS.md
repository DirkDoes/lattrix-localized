...
- The web container keeps `node_modules` in its own volume. After changing an npm dependency on the host, run `docker compose exec web npm install` and restart the web service before visually checking the new component version.
